...
if uploaded_file:
    ...
    if uploaded_file.type == 'application/vnd.ms-excel':  # Check for XLSX file type
        ...
        df = pd.read_csv(uploaded_file, engine='python')  # Use pd.read_csv for CSV files
...
if st.button("PV1"):
    ...
    csv = pivot_table_pv1.to_csv(index=True, sep=',')  # Specify the delimiter as a comma
    st.download_button(
...
if st.button("PV2"):
    ...
    csv = pivot_table_pv2.to_csv(index=True, sep=',')  # Specify the delimiter as a comma
    st.download_button(
...
if st.button("PV3"):
    ...
    csv = pivot_table_pv3.to_csv(index=True, sep=',')  # Specify the delimiter as a comma
    st.download_button(
